...
// class constructor
    function moneyorder() {
      ...
      $this->payment = MODULE_PAYMENT_MONEYORDER_USE_PAYMENTS;	  	  
...
// and the choosen method is not in selected shipping method
	  ...
	  global $shipping;
	    if (tep_not_null(MODULE_PAYMENT_MONEYORDER_USE_PAYMENTS)) {
		  if ( MODULE_PAYMENT_MONEYORDER_USE_PAYMENTS != 'all' ) {
	        
			$ship_method=preg_split("/_+/", $shipping['id']) ;
		    $ship_allowed=preg_split("/;+/", MODULE_PAYMENT_MONEYORDER_USE_PAYMENTS) ;		  
	        //$ship_method=split ("_",$shipping['id']);
		    //$ship_allowed=split (";",MODULE_PAYMENT_MONEYORDER_USE_PAYMENTS);
...
// END Shipping dependency	 	  
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . $multi_stores_config . " where configuration_key = 'MODULE_PAYMENT_MONEYORDER_STATUS'");
    ...
    function install() {
      ...
      tep_db_query("insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Make Payable to:', 'MODULE_PAYMENT_MONEYORDER_PAYTO', '', 'Who should payments be made payable to?', '6', '1', now());");
      tep_db_query("insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_MONEYORDER_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    ...
    function keys() {
      return array('MODULE_PAYMENT_MONEYORDER_STATUS', 'MODULE_PAYMENT_MONEYORDER_ZONE', 'MODULE_PAYMENT_MONEYORDER_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYORDER_SORT_ORDER', 'MODULE_PAYMENT_MONEYORDER_PAYTO', 'MODULE_PAYMENT_MONEYORDER_USE_PAYMENTS');
    }
  }
?>