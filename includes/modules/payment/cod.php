	  ...
	  global $shipping;
	    if (tep_not_null(MODULE_PAYMENT_COD_USE_PAYMENTS)) {
		  if ( MODULE_PAYMENT_COD_USE_PAYMENTS != 'all' ) {
			...
			$ship_allowed=preg_split("/;+/", MODULE_PAYMENT_COD_USE_PAYMENTS) ;
...
// END Shipping dependency	  
    ...
    function check() {
      ...
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . $multi_stores_config . " where configuration_key = 'MODULE_PAYMENT_COD_STATUS'");
    ...
    function install() {
      ...
      tep_db_query("insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_COD_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
      ...
      tep_db_query("insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Use Payment in combination with Shipping.', 'MODULE_PAYMENT_COD_USE_PAYMENTS', 'all', 'select shipping method to be used with this payment option. ', '6', '0','tep_cfg_select_payment(' , now())");
