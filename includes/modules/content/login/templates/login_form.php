          ...
          <p class="alert alert-success"><?php echo MODULE_CONTENT_LOGIN_TEXT_RETURNING_CUSTOMER; ?></p>
             <?php echo tep_draw_form('login', tep_href_link(FILENAME_LOGIN, 'action=process', 'SSL'), 'post', '', true); ?>
                 <div class="form-group">
                  ...
                  <p class="text-right"><?php echo tep_draw_button(IMAGE_BUTTON_LOGIN, 'log-in', null, 'primary', NULL, 'btn-success btn-block'); ?></p>
             </form>
      </div>
   </div>
   <p><?php echo '<a class="btn btn-info" role="button" href="' . tep_href_link(FILENAME_PASSWORD_FORGOTTEN, '', 'SSL') . '">' . MODULE_CONTENT_LOGIN_TEXT_PASSWORD_FORGOTTEN . '</a>'; ?></p>
</div>