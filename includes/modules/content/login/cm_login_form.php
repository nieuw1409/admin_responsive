...
//        $customer_query = tep_db_query("select customers_id, customers_password from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($email_address) . "' limit 1");
        $customer_query = tep_db_query("select customers_id, customers_firstname, customers_group_id, customers_password, customers_email_address, customers_default_address_id, customers_specific_taxes_exempt from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($email_address) . "' limit 1");
...
// bof customer suspend
          $check_customer_suspended_query = tep_db_query("select customers_id, customers_firstname, customers_lastname, customers_password, customers_email_address, customers_default_address_id from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($email_address) . "' and customers_suspended = 'False'");

          if (!tep_db_num_rows($check_customer_suspended_query)) {
             $error = true;
             $err_info = 'suspended';
          } else {
...
// bof suspended customer	  
//      if ($error == true) {
//        $messageStack->add('login', MODULE_CONTENT_LOGIN_TEXT_LOGIN_ERROR);
//      }

      if ($error == true && $err_info == '') {
         $messageStack->add('login', MODULE_CONTENT_LOGIN_TEXT_LOGIN_ERROR );
      } elseif ($error == true && $err_info == 'suspended') {
         $messageStack->add('login', MODULE_CONTENT_TEXT_LOGIN_ERROR_SUSPENDED );
      }
...
// eof customer suspend	  
    ...
    function install() {
      ...
      tep_db_query("insert into " . $multi_stores_config  . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_CONTENT_LOGIN_FORM_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
   ...
   function enable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'True' where configuration_key = 'MODULE_CONTENT_LOGIN_FORM_STATUS'");
            $this->enabled = (MODULE_CONTENT_LOGIN_FORM_STATUS == 'True');
    ...
    function disable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'False' where configuration_key = 'MODULE_CONTENT_LOGIN_FORM_STATUS'");
            $this->enabled = (MODULE_CONTENT_LOGIN_FORM_STATUS == 'False');
