...
*/
  ...
  class cm_sc_stock_notice {
    ...
    function execute() {
      ...
      if ($cart->count_contents() > 0 && $any_out_of_stock == 1) {
        ...
        if (STOCK_ALLOW_CHECKOUT == 'true') {
          $sc_stock_notice = '<div class="alert alert-warning">' . OUT_OF_STOCK_CAN_CHECKOUT . '</div>' . PHP_EOL ;
        } else {
          $sc_stock_notice = '<div class="alert alert-danger">' . OUT_OF_STOCK_CANT_CHECKOUT . '</div>' . PHP_EOL ;
        }
    ...
    function install() {
      ...
      tep_db_query("insert into " . $multi_stores_config  . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_CONTENT_SC_STOCK_NOTICE_SORT_ORDER', '1000', 'Sort order of display. Lowest is displayed first.', '6', '3', now())");
    }
    ...
    function enable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'True' where configuration_key = 'MODULE_CONTENT_SC_STOCK_NOTICE_STATUS'");
            $this->enabled = (MODULE_CONTENT_SC_STOCK_NOTICE_STATUS == 'True');
    ...
    function disable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'False' where configuration_key = 'MODULE_CONTENT_SC_STOCK_NOTICE_STATUS'");
            $this->enabled = (MODULE_CONTENT_SC_STOCK_NOTICE_STATUS == 'True');
