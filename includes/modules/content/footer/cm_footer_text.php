...
*/
  ...
  class cm_footer_text {
    ...
    function execute() {
	  ...
	  $footer_text_text  = constant( 'MODULE_CONTENT_FOOTER_TEXT_TEXT_' . strtoupper( $language ) );
	  $footer_text_title = constant( 'MODULE_CONTENT_FOOTER_TEXT_TITLE_' . strtoupper( $language ) );
    ...
    function install() {
      ...
      tep_db_query("insert into " .  $multi_stores_config   . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_CONTENT_FOOTER_TEXT_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");

   	foreach( $this->languages_array as $language_name ) {
        tep_db_query( "insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ( '" . ucwords( $language_name ) . " Title', 'MODULE_CONTENT_FOOTER_TEXT_TITLE_" . strtoupper( $language_name ) . "', 'Generic Title', 'Enter the title that you want on your box in " . $language_name . "', '6', '10', now())" );
        tep_db_query( "insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ( '" . ucwords( $language_name ) . " Contents', 'MODULE_CONTENT_FOOTER_TEXT_TEXT_" . strtoupper( $language_name ) . "', 'Generic Contents', 'Enter the contents that you want in your box in " . $language_name . "', '6', '20', 
		'tep_draw_textarea_ckeditor(\'configuration[MODULE_CONTENT_FOOTER_TEXT_TEXT_" . strtoupper( $language_name ) . "]\', false, 115, 100, tep_get_config_value( MODULE_CONTENT_FOOTER_TEXT_TEXT_" . strtoupper( $language_name ) . " ),  tep_get_text_class() , ' , now())" );
    ...
    function keys() {
      ...
      foreach( $languages as $this_language ) {
    	...
    	$keys[] = 'MODULE_CONTENT_FOOTER_TEXT_STATUS';
    	$keys[] = 'MODULE_CONTENT_FOOTER_TEXT_CONTENT_WIDTH';
    	$keys[] = 'MODULE_CONTENT_FOOTER_TEXT_SORT_ORDER';
  
		
    	foreach( $this->languages_array as $language_name ) {
    	  $keys[] = 'MODULE_CONTENT_FOOTER_TEXT_TITLE_' . strtoupper( $language_name );
    	  $keys[] = 'MODULE_CONTENT_FOOTER_TEXT_TEXT_' . strtoupper( $language_name );
    	}
 ...
 //     return array('MODULE_CONTENT_FOOTER_TEXT_STATUS', 'MODULE_CONTENT_FOOTER_TEXT_CONTENT_WIDTH', 'MODULE_CONTENT_FOOTER_TEXT_SORT_ORDER');
    ...
    function enable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'True' where configuration_key = 'MODULE_CONTENT_FOOTER_TEXT_STATUS'");
            $this->enabled = (MODULE_CONTENT_FOOTER_TEXT_STATUS == 'True');
    ...
    function disable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'False' where configuration_key = 'MODULE_CONTENT_FOOTER_TEXT_STATUS'");
            $this->enabled = (MODULE_CONTENT_FOOTER_TEXT_STATUS == 'False');
