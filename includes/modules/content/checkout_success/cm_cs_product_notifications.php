...
//              $products_displayed[$products['products_id']] = '<label class="checkbox-inline"> ' . tep_draw_checkbox_field('notify[]', $products['products_id']) . ' ' . $products['products_name'] . '</label>';
	  $products_displayed[$products['products_id']]  = '	                        <div class="form-group">' . PHP_EOL .
	                                                                                    tep_bs_checkbox_field('notify[]', $products['products_id'], $products['products_name'], 'notify_comments'.products_id, false, 'checkbox checkbox-success', '', '', 'right') .
                                                                                   '</div>' . PHP_EOL;			  
            }
          }
    ...
    function enable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'True' where configuration_key = 'MODULE_CONTENT_CHECKOUT_SUCCESS_PRODUCT_NOTIFICATIONS_STATUS'");
            $this->enabled = (MODULE_CONTENT_CHECKOUT_SUCCESS_PRODUCT_NOTIFICATIONS_STATUS == 'True');
    ...
    function disable() {
	  global $multi_stores_config;			
            tep_db_query("update " . $multi_stores_config  . " set configuration_value = 'False' where configuration_key = 'MODULE_CONTENT_CHECKOUT_SUCCESS_PRODUCT_NOTIFICATIONS_STATUS'");
            $this->enabled = (MODULE_CONTENT_CHECKOUT_SUCCESS_PRODUCT_NOTIFICATIONS_STATUS == 'False');
