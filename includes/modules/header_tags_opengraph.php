...
switch (true) {
    case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_DEFAULT):
        if ($category_depth == 'top') {
            $img = (file_exists('store_logo.png') ? tep_href_link(DIR_WS_IMAGES . 'store_logo.png', '', 'NONSSL', false, false) : '');
            ...
            $twitterArray[] = '<meta name="twitter:url" content="' . tep_href_link(FILENAME_DEFAULT, '', 'NONSSL', false) . '">';
    ...
    case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCT_REVIEWS_INFO):
        if ($_GET['products_id'] > 0) {
            ...
            if ( tep_db_num_rows($og_query) > 0 ) {
                ...
                $img = (tep_not_null($og['products_image']) ? tep_href_link(DIR_WS_IMAGES . $og['products_image'], '', 'NONSSL', false, false) : '');                
                $url = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $og['products_id'], 'NONSSL', false);
    ...
    case (defined('FILENAME_ARTICLE_INFO') && basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_ARTICLE_INFO):
        if ($_GET['articles_id'] > 0) {
            ...
            if ( tep_db_num_rows($og_query) > 0 ) {
                ...
                $img = (tep_not_null($og['articles_image']) ? tep_href_link(DIR_WS_IMAGES . $og['articles_image'], '', 'NONSSL', false, false) : '');
                $url = tep_href_link(FILENAME_ARTICLE_INFO, 'article_id=' . $og['articles_id'], 'NONSSL', false);
                ...
                if (isset($og['authors_id'][1])) $ogArray['author'] = tep_href_link(FILENAME_ARTICLES, 'authors_id=' . $og['authors_id'], 'NONSSL', false);
