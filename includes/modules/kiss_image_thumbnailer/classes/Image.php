<?php
  ...
  * @lastmod $Date:: 2012-06-30 15:36:23 +0100 (Sat, 30 Jun 2012)       $:  Date of last commit
  * @version $Rev:: 3                                                   $:  Revision of last commit
  * @Id $Id:: Image.php 3 2012-06-30 14:36:23Z FWR Media                $:  Full Details
  */
  class Image {
    /**
    ...
    * @param mixed $filters
    */
    public function save( $save_in = null, $quality = 75, $filters = null ) {
    ...
    */
    ...
    protected function _handleTransparentColor( $image = null ) {
      ...
      if (($this->_mime_type == self::IMAGETYPE_GIF) || ($this->_mime_type == self::IMAGETYPE_PNG)) {
        ...
        // It's a gif and we have no transparent index so we'll apply a solid background
        } elseif ( ( $this->_mime_type == self::IMAGETYPE_GIF ) && $background ) {
          $background_color = imagecolorallocate ( $image, $this->_thumb_background_rgb['red'], $this->_thumb_background_rgb['green'], $this->_thumb_background_rgb['blue'] );
    ...
    public function resize( $max_width, $max_height ) {
      ...
      $new_image = $this->_handleTransparentColor ( $new_image );
      ...
      if ( $this->_take_resize_dimensions_as_absolute ) {
        ...
        if ( $max_width < $this->_requested_thumbnail_width || $max_height < $this->_requested_thumbnail_height ) {
          if (($this->_mime_type == self::IMAGETYPE_GIF) || ($this->_mime_type == self::IMAGETYPE_PNG)) {
            ...
            $thumb_background = $this->_handleTransparentColor ( $thumb_background );
          } else {
            $thumb_background = imagecreatetruecolor ( $this->_requested_thumbnail_width, $this->_requested_thumbnail_height );
