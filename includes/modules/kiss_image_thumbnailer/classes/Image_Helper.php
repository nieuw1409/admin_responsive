  ...
  class Image_Helper extends ArrayObject {
    ...
    */
    protected function _build() {
      ...
      $this->_html = '<img width="' . $width . '" height="' . $height . '" src="' . $this->src . '" title="' . $alt_title . '" alt="' . $alt_title . '"  itemprop="image"';
      
// bof responsive design	  
	  $this->_html .= ' class="';

      if (tep_not_null( $this->responsive) && ($this->responsive === true)) {
        $this->_html .= 'img-responsive';
      }

      if (tep_not_null($this->bootstrap_css)) $image .= ' ' . $this->bootstrap_css;

      $this->_html .= '"';
// eof responsive design	  
      if ( false !== $parameters ) $this->_html .= ' ' . tep_output_string( $parameters ); //tep_output_string( $parameters );
