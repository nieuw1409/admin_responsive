<?php
  ...
  * @param string $target_language_code - 2 letter code of target language in flag link
  * @return string - URL
  */
  function osc_href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true, $target_language_code = false ) {
    ...
    if ( tep_not_null( $parameters ) ) {
      $link .= $page; 
      $link .= ( $multi_language_valid ? usu5_multi_language( 'left', $target_language_code ) : null ) . '?' . tep_output_string( $parameters );
      $separator = '&';
    } else {
      $link .= $page;
      $link .= ( $multi_language_valid ? usu5_multi_language( 'left', $target_language_code ) : null );
      $separator = '?';
    }
...
// Add the session ID when moving from different HTTP and HTTPS servers, or when SID is defined
  ...
  */
  function do_homepage_redirect( $url = false ) { 
    if ( defined( 'USU5_HOME_PAGE_REDIRECT' ) && ( USU5_HOME_PAGE_REDIRECT == 'true' )
                                              && ( USU5_ENABLED == 'true' ) && (mobile_session() === false) ) {
      if ( false === $url ) {
  ...
  */
  function usu5_multi_language( $separator = false, $target_language_code = false ) {
  
    if( false === $target_language_code) {
    	$language_code = Usu_Main::i()->getVar( 'current_language', 'code' );
    }
    else {
