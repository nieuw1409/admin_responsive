...
*/
  ...
  class ot_fixed_payment_chg {
    ...
    function ot_fixed_payment_chg() {
	  ...
	  $this->enabled = ((MODULE_FIXED_PAYMENT_CHG_STATUS == 'true') ? true : false);
...
function process() {
	...
	global $multi_stores_config ;
      if (!isset($this->check)) {
        $check_query = tep_db_query("select configuration_value from " . $multi_stores_config . " where configuration_key = 'MODULE_FIXED_PAYMENT_CHG_STATUS'");
...
//BOF angol telepítõ eleje , english install 
    function install() {
	  GLOBAL $multi_stores_config ;
      ...
      tep_db_query("insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_FIXED_PAYMENT_CHG_SORT_ORDER', '3', 'Display sort order.', '6', '2', now())");
...
//EOF angol telepítõ vége , english install 
    function remove() {
      GLOBAL $multi_stores_config ;	
      $keys = '';
