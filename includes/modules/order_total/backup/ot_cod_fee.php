    ...
    function process() {
      ...
      if (MODULE_ORDER_TOTAL_COD_STATUS == 'true') {
        ...
        if ($GLOBALS['payment'] == 'cod') {
          ...
          //if ($shipping['id'] == 'flat_flat') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_FLAT);
          //if ($shipping['id'] == 'item_item') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_ITEM);
          //if ($shipping['id'] == 'table_table') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_TABLE);
          //if ($shipping['id'] == 'ups_ups') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_UPS);
          //if ($shipping['id'] == 'usps_usps') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_USPS);
          //if ($shipping['id'] == 'zones_zones') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_ZONES);
          //if ($shipping['id'] == 'ap_ap') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_AP);
          //if ($shipping['id'] == 'dp_dp') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_DP);
		  //if (substr_count($shipping['id'], 'servicepakke') !=0) $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_SERVICEPAKKE);
		  //if (substr_count($shipping['id'], 'canadapost') !=0) $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_CANADA_POST);
          //if ($shipping['id'] == 'dhl_dhl') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_DHL);
          if ($shipping['id'] == 'flat_flat') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_FLAT);
          if ($shipping['id'] == 'item_item') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_ITEM);
          if ($shipping['id'] == 'table_table') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_TABLE);
          if ($shipping['id'] == 'ups_ups') $cod_zones = preg_split("/:,/", MODULE_ORDER_TOTAL_COD_FEE_UPS);
...
//                                  'value' => '');
	...
	global $multi_stores_config ;
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . $multi_stores_config . " where configuration_key = 'MODULE_ORDER_TOTAL_COD_STATUS'");
	...
	global $multi_stores_config ;	
      ...
      tep_db_query("insert into " . $multi_stores_config . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_COD_SORT_ORDER', '4', 'Sort order of display.', '6', '2', now())");
