...
// eof sppc
      ...
      if ( isset($_category_tree_data) ) {
        ...
        $categories_query = tep_db_query("select c.categories_id, c.parent_id, c.categories_image, cd.categories_name, c.categories_status 
      ...
      if(isset($this->_data[$parent_id])) {
        foreach($this->_data[$parent_id] as $category_id => $category) {
          ...
          if(isset($this->_data[$category_id]) && (($this->max_level == '0') || ($this->max_level > $level + 1))) {
		    ...
		    $root_link_title =  '<span class="hidden-xs"><span class="' . glyphicon_icon_to_fontawesome( "th-list" ) . '"></span>&nbsp;' . $link_title . '</span><li class="divider"></li>'; //glyphicon glyphicon-th-list
