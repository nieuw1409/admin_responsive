...
*/
  class pad_base {
	...
	var $attributes_availability_nostock;
...
*/
    function pad_base($products_id=0, $attr_availability_id=0) {
	  ...
	  $this->attributes_availability_nostock = $attr_availability_id ;
    }
      ...
      if (sizeof($attributes)>0) {
        foreach ($attributes as $stocked) {
          //$out .= '<tr><td align="right" class=main><strong>' . $stocked['oname'] . ":</strong></td><td class=main>" . tep_draw_pull_down_menu('id['.$stocked['oid'].']',array_values($stocked['ovals']),$stocked['default']) . "</td></tr>\n";
            ...
            $out .= '<tr>
			             <td class="label label-info col-md-2"><strong>' . $stocked['oname'] . ':</td>
				         <td class="col-md-10">' . tep_bootstrap_pull_down_menu('id['.$stocked['oid'].']',array_values($stocked['ovals']), 'id'.$stocked['oid'], $stocked['default'] ). '</td>
				     </tr>' . PHP_EOL ; //						 tep_draw_pull_down_menu('id['.$stocked['oid'].']',array_values($stocked['ovals']),$stocked['default']) . '</td>
...
*/
    function _draw_nonstocked_attributes() {
      ...
      {
        $out.='<tr><td align="right" class=main><strong>'.$nonstocked['oname'].":</strong></td><td class=main>".tep_bootstrap_pull_down_menu('id['.$nonstocked['oid'].']',$nonstocked['ovals'], 'id'.$nonstocked['oid'], $nonstocked['default'])."</td></tr>\n"; // tep_draw_pull_down_menu('id['.$nonstocked['oid'].']',$nonstocked['ovals'],$nonstocked['default'])
      }
      return $out;
    }
/*
...
// sppc price mod
		if ( $customer_group_id != '0') { 
            $pag_query = tep_db_query("select products_attributes_id, options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES_GROUPS . " where products_attributes_id = '" . $products_options['products_attributes_id'] . "' AND customers_group_id = '" . $customer_group_id . "'");
	        $cg_attr_prices = tep_db_fetch_array($pag_query) ;
		    if (tep_not_null($cg_attr_prices)) {
		       $products_options['price_prefix'] = $cg_attr_prices['price_prefix'];
		       $products_options['options_values_price'] = $cg_attr_prices['options_values_price'];		 		 
		    }
      ...
      foreach ($attributes[$oidindex]['ovals'] as $attrib) {
        ...
        if (isset($attributes[$oidindex+1])) {
          ...
          if (!$is_out_of_stock | ($showoos == true)) {
            switch ($markoos) {
              case 'Left':   $newtext=($is_out_of_stock ? $products_availability_name.' - ' : '').substr($newtext,2);
                             break;
              case 'Right':  $newtext=substr($newtext,2).($is_out_of_stock ? ' - '. $products_availability_name  : '');
                             break;
