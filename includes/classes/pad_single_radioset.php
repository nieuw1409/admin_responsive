...
*/
  require(DIR_WS_CLASSES . 'pad_single_dropdown.php');
...
*/
    function _draw_stocked_attributes() {
      ...
      if (sizeof($attributes)>0) {
		...
		if ( MOBILE_QTPRO_ENABLED == 'true' ) {
              $out.= '<div data-role="fieldcontain"><fieldset data-role="controlgroup"> ';
        }        
        ...
        foreach ($combinations as $combindex => $comb) {
          ...
          if ( MOBILE_QTPRO_ENABLED == 'true' ) {
			  ...
			  $out .=tep_mobile_radio_field(  'attrcomb', // $name, 
			                                  $combinations[$combindex]['id'],  //$value = '', 
											  ($combindex==$selected_combination),  //$checked = false, 
											  '',                                  //$parameters = '', 
											  $comb['text'],                                  //$label= '', 
											  'attrcomb'.$combindex,               //$id_text = '', 
											  '',                                  //$initial_text = '', 
											  $combname ) ;                         //$legend = '' )
	           $combname = '' ;
          } else {
