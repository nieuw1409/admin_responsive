...
//    $information_query = tep_db_query("select information_id, information_title, parent_id from " . TABLE_INFORMATION . " where visible = '1' and language_id = '" . (int)$languages_id ."' and information_group_id = '" . (int)$information_group_id . "' ORDER BY sort_order");
    $information_query = tep_db_query("select information_id, information_title, parent_id from " . TABLE_INFORMATION . " where visible = '1' and language_id = '" . (int)$languages_id ."' and information_group_id = '" . (int)$information_group_id . "'  and stores_id = '" . SYS_STORES_ID . "' ORDER BY sort_order");
// eof multi store	
    ...
    while ($element = each($information_tree)) {
      if (!isset($information_tree[$element['key']]['parent_id']) || ($information_tree[$element['key']]['parent_id'] == 0)) {
        ...
        if (((isset($_GET['info_id'])) && ($_GET['info_id'] == $element['key'])) || ($parent_child_selected == $element['key'])) {
          $informationString .= '<br /><a href="' . tep_href_link(FILENAME_INFORMATION, 'info_id=' . $element['key']) . '"><strong>' . $information_tree[$element['key']]['info_title'] . '</strong><br /></a>';
        } else {
          $informationString .= '<br /><a href="' . tep_href_link(FILENAME_INFORMATION, 'info_id=' . $element['key']) . '">' . $information_tree[$element['key']]['info_title'] . '<br /></a>';
          //Sitemap only
          $sitemapString .= '<li><a href="' . tep_href_link(FILENAME_INFORMATION, 'info_id=' . $element['key']) . '">' . $information_tree[$element['key']]['info_title'] . '<br /></a></li>' . PHP_EOL;
        }
        ...
        for ($i = 1; $i < ($count_child+1); $i++) {
          if ($child_information[$i]['parent_info_id'] == $element['key']) {
            if ($ul == false) {
              $sitemapString .= '<ul>' . PHP_EOL;
              $ul = true;
            }
            $sitemapString .= '<br /><li><a href="' . tep_href_link(FILENAME_INFORMATION, 'info_id=' . $child_information[$i]['child_info_id']) . '">' . $information_tree[$child_information[$i]['child_info_id']]['info_title'] . '</a></li><br />' . PHP_EOL;
          }
          ...
          for ($i = 0; $i < ($count_child); $i++) {
            if ($child_information[$i]['parent_info_id'] == $element['key'])
              ...
              if ((isset($_GET['info_id'])) && ($_GET['info_id'] == $child_information[$i]['child_info_id'])) {
                $informationString .= '&nbsp;&nbsp;<br /><a href="' . tep_href_link(FILENAME_INFORMATION, 'info_id=' . $child_information[$i]['child_info_id']) . '"><strong>' . $information_tree[$child_information[$i]['child_info_id']]['info_title'] . '</strong><br /></a>';
              } else {
                $informationString .= '&nbsp;&nbsp;<br /><a href="' . tep_href_link(FILENAME_INFORMATION, 'info_id=' . $child_information[$i]['child_info_id']) . '">' . $information_tree[$child_information[$i]['child_info_id']]['info_title'] . '</a><br />';
              }
          }
        }
      }
...
// Define customer greetings
  function tep_information_customer_greeting_define() {
    ...
    if ($category_depth == 'top') {
      ...
      $information_query = tep_db_query("select information_title, information_description from " . TABLE_INFORMATION . " where language_id = '" . (int)$languages_id . "' and information_group_id = '" . (int)$information_group_id . "'  and stores_id = '" . SYS_STORES_ID . "'");  // MULTI STORES
