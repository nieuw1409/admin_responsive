...
function ReadCacheHeaderTags(&$header_tags_array, $filename, $id) {
   ...
   return ( (HEADER_TAGS_ENABLE_CACHE == 'None') ? false : ReadCacheFromDB($header_tags_array, 'header_tags_' . $filename . '_' . $language . '_cache_id_' . $id . SYS_STORES_ID ) ); 
}
...
function WriteCacheHeaderTags($header_tags_array, $filename, $id)
{
   ...
   if (HEADER_TAGS_ENABLE_CACHE != 'None') {   
      ...
      if (tep_not_null($cache_output)) {
          WriteCacheToDB($cache_output, 'header_tags_' . $filename . '_' . $language . '_cache_id_' . $id . SYS_STORES_ID);
      }
   }
}
