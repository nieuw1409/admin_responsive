...
//       and the new languages are not automatically generated by languages.php
  ...
  function tep_update_information_languages($language_id = 0) {
    global $languages, $languages_id, $gID, $multi_stores_id ;
...
// eof multi stores	  
  ...
  function tep_get_information_list($language_id = 0) {
    global $languages_id, $gID, $multi_stores_id ;
    if ($language_id == 0) $language_id = $languages_id;

// bof multi stores	
    $information_query = tep_db_query("select * from " . TABLE_INFORMATION . " where language_id = '" . (int)$language_id . "' and information_group_id = '" . (int)$gID . "' and stores_id = '" . (int)$multi_stores_id . "' order by sort_order");	
// eof multi stores	
...
// WT
  function tep_get_information_entry($information_id, $language_id = 0, $column = '') {
    global $languages_id, $multi_stores_id ;
    if ($language_id == 0) $language_id = $languages_id;
...
// eof multi stores		
  ...
  switch($action) {
    ...
    case "AddSure":
	  ...
	  $sql_data_array['stores_id'] = $multi_stores_id ;  // multi stores
      ...
      if ((int)$_POST['information_id']) {
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          if (count($sql_data_array) > 0) {
            ...
            if (tep_db_perform(TABLE_INFORMATION, $sql_data_array, 'update', "information_id = '" . (int)$_POST['information_id'] . "' and language_id = '" . (int)$language_id . "'  and stores_id = '" . (int)$multi_stores_id . "'")) { // multi stores
  ...
  switch($action) {
    ...
    case "Edit":
      if ($information_id = $_REQUEST['information_id']) {
        ...
        echo tep_draw_bs_form('', FILENAME_INFORMATION_MANAGER, 'information_action=Update', 'post', 'class="form-horizontal" role="form"', 'id_edit_info_pages');
...
//        echo "</form></td></tr>";
		...
		$contents .= '          <div class="panel-heading">' . $delete['information_title'] . '</div>' . PHP_EOL;
		$contents .= '          <div class="panel-body">' . PHP_EOL;											
		$alertClass .= ' alert alert-danger';
		$contents .= '                      ' . tep_draw_form('info_manager', FILENAME_INFORMATION_MANAGER, "information_action=DelSure&information_id=" . $delete['information_id']) . PHP_EOL;
		$contents .=                            tep_draw_hidden_field('information_id', $information_id) . PHP_EOL ;
		$contents .=                            tep_draw_hidden_field('gID', $gID) . PHP_EOL ;
        $contents .= '                        <div class="col-xs-12 col-sm-8 col-md-8">' . PHP_EOL;
        $contents .= '                          <p>' . DELETE_CONFIRMATION_ID_INFORMATION . '<br />' . $delete['information_title']   . '</p>' . PHP_EOL;
        ...
        $contents .= '                          ' . tep_draw_bs_button(IMAGE_DELETE, 'ban-circle', null, null, null, 'btn-danger') . '<br>' . 
									                tep_draw_bs_button(IMAGE_CANCEL, 'remove',  tep_href_link(FILENAME_INFORMATION_MANAGER, 'gID=' . $gID), null, null, 'btn-default text-danger') . PHP_EOL;
        $contents .= '                        </div>' . PHP_EOL;
