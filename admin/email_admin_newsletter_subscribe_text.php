        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
	      ...
	      if (EMAIL_USE_HTML == 'true'){	
		     ...
		     $admin_password = preg_replace('/\r\n|\r|\n/', ' ', $admin_password);
 		     ...
 		     if (tep_db_query("update " . TABLE_EMAIL_ORDER_TEXT . " set eorder_text_one = '" . $admin_password . "', eorder_title = '" . $admin_password_title . "'  where eorder_text_id = '" . _SYS_EMAIL_HTML_SUBSCRIBE_NEWSLETTER_ADMIN . "' and language_id='" . $languages_id . "' and stores_id='" . $multi_stores_id . "'")) {
               $messageStack->add(SUCCESS_EMAIL_ORDER_TEXT, 'success');
             } else {
   			   $messageStack->add(ERROR_EMAIL_ORDER_TEXT, 'error'); 
    	     } 
          } else {
 		     if (tep_db_query("update " . TABLE_EMAIL_ORDER_TEXT . " set eorder_text_one = '" . $admin_password . "', eorder_title = '" . $admin_password_title . "'  where eorder_text_id = '" . _SYS_EMAIL_NORMAL_SUBSCRIBE_NEWSLETTER_ADMIN . "' and language_id='" . $languages_id . "' and stores_id='" . $multi_stores_id . "'")) {
               $messageStack->add(SUCCESS_EMAIL_ORDER_TEXT, 'success');
             } else {
   			   $messageStack->add(ERROR_EMAIL_ORDER_TEXT, 'error'); 
    	     }     	
  ...
  <table border="0" width="100%" cellspacing="0" cellpadding="2">
		  ...
		  <div class="col-md-6 col-xs-12">
              <ul class="list-group">
                  ...
                  <li class="list-group-item"><-SYS_SUBSCRIBER_EMAIL_ADDRESS->                   <?php echo ' :  ' . POPUP_SUBSCRIBER_EMAIL_ADRESS ?></li>		    		  
                  <li class="list-group-item"><-SYS_SUBSCRIBER_NAME->                            <?php echo ' :  ' . POPUP_SUBSCRIBER_NAME ?></li>				  
                  <li class="list-group-item"><-SYS_SUBSCRIBER_ID->                              <?php echo ' :  ' . POPUP_SUBSCRIBER_ID ?></li>				  
              </ul> 
          </div> 					
		  <div class="col-md-6 col-xs-12">
              <ul class="list-group">
                  <li class="list-group-item"><-SYS_SUBSCRIBER_CUSTOMERS_GROUP_ID->              <?php echo ' :  ' . POPUP_SUBSCRIPTION_CUSTOMERS_GROUP_ID ?></li>				   
                  <li class="list-group-item"><-SYS_SUBSCRIBER_STORES_ID->                       <?php echo ' :  ' . POPUP_SUBSCRIPTION_STORES_ID ?></li>	   			   	  
                  <li class="list-group-item"><-SYS_SUBSCRIBER_DATE_CREATION->                   <?php echo ' :  ' . POPUP_SUBSCRIPTION_DATE_CREATION ?></li>				  
              </ul> 
          </div> 
          <div class="clearfix"></div>		  		  
		  <hr>
            ...
            <table class="table table-condensed table-striped">
              ...
              <tbody>				
<?php			                  
    			$contents_edit_password_forgotten       .=  tep_draw_bs_form('password_forgottenform', FILENAME_EMAIL_ADMIN_NEWSLETTER_SUBSCRIBE_TEXT,'action=update', 'post', '', 'id_form_password_forgotten_admin_description')  ;				 
				...
				for ($i = 0, $n = sizeof($languages); $i < $n; $i++) { 
                   ...
                   if (EMAIL_USE_HTML == 'true'){
                      $sql=tep_db_query("SELECT * FROM " . TABLE_EMAIL_ORDER_TEXT . " where eorder_text_id = '" . _SYS_EMAIL_HTML_SUBSCRIBE_NEWSLETTER_ADMIN . "'  AND language_id = '" . $languages_id . "' and stores_id='" . $multi_stores_id . "'");
                   } else {    
                      $sql=tep_db_query("SELECT * FROM " . TABLE_EMAIL_ORDER_TEXT . " where eorder_text_id = '" . _SYS_EMAIL_NORMAL_SUBSCRIBE_NEWSLETTER_ADMIN . "'  AND language_id = '" . $languages_id . "' and stores_id='" . $multi_stores_id . "'");
                   }  
