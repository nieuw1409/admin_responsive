...
*/
  ...
  if (isset($_POST['action']) && $_POST['action'] == 'test')
  {
    ...
    for ($i = 0; $i < count($files); ++$i) {
        if (! file_exists($files[$i]) && (strpos($files[$i], "install.php") === FALSE && strpos($files[$i], "update.php") === FALSE)) {
            if (! $found) {
              $results[] = ERROR_HEADING_MISSING_FILE . $explain;;
              $found = true;
            }
            $results[] = sprintf(ERROR_MISSING_FILE, $files[$i]);
        } else if (file_exists($files[$i]) && (strpos($files[$i], "install.php") !== FALSE || strpos($files[$i], "update.php") !== FALSE)) {
            $extraFiles[] = $files[$i];
            $found = true;
        }  
    }
    ...
    while ($check = tep_db_fetch_array($check_query))
    {
      if (! $dbError)
      {
        ...
        $results[] = sprintf(ERROR_DEFAULT_ROOT_TEXT_PRESENT, $check['page_title'], $check['page_name'], $check['language_id']);
        $dbError = true;       
      }
      else
        $results[] = sprintf(ERROR_DEFAULT_ROOT_TEXT_PRESENT, $check['page_title'], $check['page_name'], $check['language_id']);
    ...
    for ($i = 0; $i < count($languages); ++$i)
    {
      ...
      while ($check = tep_db_fetch_array($check_query))
      {
        if (! $dbError)
        {
          ...
          $results[] = '<a class="htc_Link" href="' . tep_href_link(FILENAME_CATEGORIES, 'action=new_product&pID=' . $check['products_id']) . '">' . sprintf(ERROR_DUPLICATE_TITLE_LANGUAGE, $languages[$i]['name']) . '</a>';
          $results[] = sprintf(ERROR_DUPLICATE_TITLE_META_DESCRIPTION, $check['products_head_title_tag']);
          $dbError = true;
    ...
    if (isset($_POST['show_limit_messages']) && $_POST['show_limit_messages'] == 'on')
    {
      ...
      {    
        ...
        while ($check = tep_db_fetch_array($check_query))
        {
          $msg = (strlen($check['products_head_desc_tag']) < 60) ? ERROR_TITLE_LENGTH_SHORT : ERROR_TITLE_LENGTH_LONG;
          
          if (! $dbError)
          {
            $results[] = ERROR_HEADING_TITLE_LENGTH . $explain;;
            $results[] = sprintf(ERROR_DUPLICATE_TITLE_LANGUAGE, $languages[$i]['name']);
            $results[] = sprintf($msg, '<a class="htc_Link" href="' . tep_href_link(FILENAME_CATEGORIES, 'action=new_product&pID=' . $check['products_id']) . '">' . (tep_not_null($check['products_head_title_tag']) ? $check['products_head_title_tag'] : ERROR_NO_NAME) . '</a>',   strlen($check['products_head_title_tag']));
            $dbError = true;       
          }
          else
          {
    ...
    if (isset($_POST['show_limit_messages']) && $_POST['show_limit_messages'] == 'on')
    {
      ...
      {      
        ...
        while ($check = tep_db_fetch_array($check_query))
        {
          $msg = (strlen($check['products_head_desc_tag']) < 16) ? ERROR_DESCRIPTION_LENGTH_SHORT : ERROR_DESCRIPTION_LENGTH_LONG;
          
          if (! $dbError)
          {
            $results[] = ERROR_HEADING_DESCRIPTION_LENGTH . $explain;;
            $results[] = sprintf(ERROR_DUPLICATE_TITLE_LANGUAGE, $languages[$i]['name']);          
            $results[] = sprintf($msg, '<a class="htc_Link" href="' . tep_href_link(FILENAME_CATEGORIES, 'action=new_product&pID=' . $check['products_id']) . '">' . (tep_not_null($check['products_head_title_tag']) ? $check['products_head_title_tag'] : ERROR_NO_NAME) . '</a>',   strlen($check['products_head_desc_tag']));
            $dbError = true;       
          }
          else
          {
    ...
    {
      if (($p1 = strpos($lines[$idx], "//")) !== FALSE && ($p2 = strpos(strtolower($lines[$idx]), ".php")) !== FALSE && $p1 < $p2)
      {
        $filesCnt++;  
    ...
    if (($dbCnt = tep_db_num_rows($check_query)) != $filesCnt)
    {
       while ($check = tep_db_fetch_array($check_query))
       {
         if (strpos($check['page_name'], "?") !== FALSE)   //check for pseudo pages
           ...
           $dbArray[] = $check['page_name']; 
       }
       
       if ($dbCnt != $filesCnt)
       {
          $results[] = ERROR_HEADING_COUNT_MISMATCH . $explain;;
          $results[] = sprintf(ERROR_COUNT_MISMATCH, tep_db_num_rows($check_query), $filesCnt);  
          $dbError = true;
          ...
          else
          {
            ...
            if ($found)
             $results[] = ' <form method="post" action="header_tags_test.php">' . tep_draw_hidden_field('action', 'delete_extra_files') . tep_draw_hidden_field('files_to_delete', urlencode(serialize($extra_db_files))) . tep_hide_session_id() . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '</form>';
         }
       }
    }
    if ($dbError) $results[] = '<br>' . tep_black_line(); 
    ...
    for ($i=0; $i < count($languages); ++$i) 
    {
      ...
      $results[] = sprintf(ERROR_LANGUAGE_MISMATCH_DB, $languages[$i]['name'],$languages[$i]['id']) . ' <form method="post" action="header_tags_test.php">' . tep_draw_hidden_field('action', 'add_languages') . tep_draw_hidden_field('language_name', $languages[$i]['name']) . tep_draw_hidden_field('language_id', $languages[$i]['id']) . tep_hide_session_id() . tep_image_submit('button_add_small.gif', IMAGE_ADD) . '</form>';
    }
...
//    else if ($notFirst && $i == 0)  //show what is there
        ...
        $results[] = sprintf(ERROR_LANGUAGE_MISMATCH_HT, $check['language_id']) . ' <form method="post" action="header_tags_test.php">' . tep_draw_hidden_field('action', 'delete_languages') . tep_draw_hidden_field('language_id', $check['language_id']) . tep_hide_session_id() . tep_image_submit('button_delete_small.gif', IMAGE_DELETE) . '</form>';
      }
      else
      {
        $found = false;
        $results[] = sprintf(ERROR_LANGUAGE_MISMATCH_PRESENT, $check['language_id'], $languages[$i]['name']);
      }  
      if ($dbError) $results[] = '<br>' . tep_black_line(); 
    }
    ...
    if (isset($_POST['check_url_page']) && substr($_POST['check_url_page'], -4) == '.php')
    {
       ...
       $args = ((isset($_POST['check_url_tags']) && tep_not_null($_POST['check_url_tags'])) ? (int)$_POST['check_url_tags'] : '');
       ...
       for ($i = 0; $i < count($languages); ++$i)
       {
           ...
           $results[] = sprintf(ERROR_META_DATA_TITLE, (tep_not_null($metaData['title'][1]) ? 'yellow' : 'red'), $metaData['title']);
           $results[] = sprintf(ERROR_META_DATA_DESC, (tep_not_null($metaData['description'][1]) ? 'yellow' : 'red'), $metaData['description']);
           $results[] = sprintf(ERROR_META_DATA_KEYWORDS, (tep_not_null($metaData['keywords'][1]) ? 'yellow' : 'red'), $metaData['keywords']);
           ...
           $results[] = sprintf(ERROR_META_DATA_TITLE, (tep_not_null($pageInfo['title']) ? 'yellow' : 'red'), $pageInfo['title']);
           $results[] = sprintf(ERROR_META_DATA_DESC, (tep_not_null($pageInfo['description']) ? 'yellow' : 'red'), $pageInfo['description']);
           $results[] = sprintf(ERROR_META_DATA_KEYWORDS, (tep_not_null($pageInfo['keywords']) ? 'yellow' : 'red'), $pageInfo['keywords']);
           
           if (! tep_not_null($metaData['description'][1]) && tep_not_null($pageInfo['description']))
           {
               $results[] = ERROR_META_DATA_COMPARE_RESULTS;
               $results[] = sprintf(ERROR_META_DATA_MISSING_CODE, $_POST['check_url_page']);
           }
           else if (! tep_not_null($metaData['description'][1]) && ! tep_not_null($pageInfo['description']))
           {
               $results[] = ERROR_META_DATA_COMPARE_RESULTS;
               $results[] = sprintf(ERROR_META_DATA_NO_TAGS, $_POST['check_url_page']);
           }
  ...
  /******************* DELETE ENTRIES IN THE DATABASE WHERE LANGUAGE DOES NOT EXIST ***********************/  
  else if (isset($_POST['action']) && $_POST['action'] == 'delete_languages')
  {
     tep_db_query("delete from " . TABLE_HEADERTAGS_DEFAULT . " where language_id = '" . (int)$_POST['language_id'] . "'");
  }
  
  /******************* ADD ENTRIES IN THE HEADER TAGS DEFAULT TABLE WHERE LANGUAGE DOES NOT EXIST ***********************/  
  else if (isset($_POST['action']) && $_POST['action'] == 'add_languages')
  {
  ...
  /******************* DELETE ENTRIES IN THE DATABASE WHERE FILES DO NOT EXISTS ***********************/  
  else if (isset($_POST['action']) && $_POST['action'] == 'delete_extra_files')
  {
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
     ...
     <tr>
      <td align="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">     
            ...
            <tr>
             <td><table border="0" width="200">
              <tr>
               ...
               <td><?php echo tep_draw_checkbox_field('show_limit_messages', '', false); ?> </td>
              </tr>
             </table></td>  
            </tr>
            <tr>
             <td><table border="0" width="70%" class="smallText">
              <tr>
               ...
               <td><?php echo tep_draw_pull_down_menu('check_url_page', $files, '', '', false);?></td>
               <td><?php echo tep_draw_pull_down_menu('check_url_id', $idsArray, '', '', false);?></td>
       ...
       <?php } } else if (isset($_POST['action']) && $_POST['action'] == 'test') {?>
       <tr><td height="10"></td></tr>     
       <tr><td><?php echo tep_black_line(); ?></td></tr>          
       <tr>
     ...
     <tr><td><?php echo tep_image(DIR_WS_IMAGES . 'pixel_trans.gif', '', '100%', '4'); ?></td></tr>
     <tr><td><?php echo tep_image(DIR_WS_IMAGES . 'pixel_black.gif', '', '100%', '4'); ?></td></tr>
     <tr>
