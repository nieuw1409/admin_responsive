...
*/
  ...
  if (file_exists(DIR_WS_FUNCTIONS . 'version_checker.php')) {
      ...
      $currentVersion = 'Header Tags SEO V 3.3.3';
      ...
      if (isset($_POST['x'])) {    
            ...
            for ($i=0; $i < count($languages); ++$i) {
                ...
                if (! $failedDupSort && isset($_POST[$name])) {
                    ...
                    $pageTags_query = tep_db_query("select * from " . TABLE_HEADERTAGS . " 
					                           where page_name like '" . tep_db_input($newfiles[$pageNumb]['text']) . "' and language_id = '" . (int)$languages[$i]['id'] . "'
											          and stores_id = '" . $multi_stores_id . "'"); // multi stores
                    ...
                    $sql_data_array = array('page_title' => tep_db_prepare_input($_POST[$name]), 
                                            ...
                                            'append_category' => ((isset($_POST[$optionID[0]]) && $_POST[$optionID[0]] == 'on') ? 1 : 0), 
                                            'append_manufacturer' => ((isset($_POST[$optionID[1]]) && $_POST[$optionID[1]] == 'on') ? 1 : 0), 
                                            'append_model' => ((isset($_POST[$optionID[2]]) && $_POST[$optionID[2]] == 'on') ? 1 : 0), 
                                            'append_product' => ((isset($_POST[$optionID[3]]) && $_POST[$optionID[3]] == 'on') ? 1 : 0), 
                                            'append_root' => ((isset($_POST[$optionID[4]]) && $_POST[$optionID[4]] == 'on') ? 1 : 0),
                                            'append_default_title' => ((isset($_POST[$optionID[5]]) && $_POST[$optionID[5]] == 'on') ? 1 : 0),
                                            'append_default_description' => ((isset($_POST[$optionID[6]]) && $_POST[$optionID[6]] == 'on') ? 1 : 0),
                                            'append_default_keywords' => ((isset($_POST[$optionID[7]]) && $_POST[$optionID[7]] == 'on') ? 1 : 0),
                                            'append_default_logo' => ((isset($_POST[$optionID[8]]) && $_POST[$optionID[8]] == 'on') ? 1 : 0),
                    ...
                    if (tep_db_num_rows($pageTags_query)) {
                        tep_db_perform(TABLE_HEADERTAGS, $sql_data_array, 'update', "page_name LIKE '" . tep_db_input($newfiles[$pageNumb]['text']) . "' 
						              and language_id = '" . (int)$languages[$i]['id'] . "' and stores_id = '" . (int)$multi_stores_id . "' "); // multi stores
                    } else {
                        $insert_sql_data = array('page_name' => $newfiles[$pageNumb]['text'], 'language_id' => $languages[$i]['id']);
        ...
        for ($x = 2; $x < count($newfiles); ++$x) {    //show the correct boxes
            for ($i=0; $i < $numbLanguages; ++$i) { //show one for each language
                $pageTags_query = tep_db_query("select page_title, page_description, page_keywords, page_logo from " . TABLE_HEADERTAGS . " 
				            where page_name like '" . tep_db_input($newfiles[$x]['text']) . "' and language_id = '" . (int)$languages[$i]['id'] . "' and stores_id = '" . (int)$multi_stores_id . "' "); // multi stores
                $pageTags = tep_db_fetch_array($pageTags_query); 
          ...
          for ($y = 2; $y < count($newfiles); ++$y) 
          {
             if (strpos($newfiles[$y]['text'], $newfiles[$x]['text']) !== FALSE) //check for pseudo pages
             {
                ...
                if ($deleted)
                {
                   for ($i=0; $i < $numbLanguages; ++$i)  //show one for each language
                   {
                      $pageDelete_query = tep_db_query("delete from " . TABLE_HEADERTAGS . " 
					      where page_name like '" . $newfiles[$y]['text'] . "' and language_id = '" . (int)$languages[$i]['id'] . "'  and stores_id =   '" . (int)$multi_stores_id . "' "); // multi stores
      ...
      for ($i=0; $i < count($languages); ++$i) 
      {
        ...
        if (isset($_POST[$name]))
        {
          ...
          $sql_data_array = array('default_title' =>       tep_db_prepare_input($_POST[$name]), 
                                  ...
                                  'meta_og' =>        ((isset($_POST[$metaOptionID[9]]) && $_POST[$metaOptionID[9]] == 'on') ? 1 : 0),
      ...
      <td><table border="0" width="100%" cellspacing="0" cellpadding="2" class="BorderedBox">
       <tr>
        <td><table border="0" width="95%" cellspacing="0" cellpadding="2">
         <tr>
          <td class="ui-widget-header ui-corner-all" valign="top"><?php echo TEXT_MULTI_STORE_NAME . $multi_stores ; ?></td>
         </tr>
		 <br />
         <tr>		 
          <td class="ui-widget-header ui-corner-all" valign="top" style="white-space:nowrap;"><?php echo $currentVersion; ?></td>		  
         </tr>
         <tr>
          <td class="ui-state-highlight smallText" valign="top"><?php echo HEADING_TITLE_SUPPORT_THREAD; ?></td>
         </tr>
        </table></td>

        <td><table border="0" width="100%">
         <tr>
          <td class="ui-state-highlight smallText" align="right"><?php echo HEADING_TITLE_AUTHOR; ?></td>
         </tr>
         <?php
         ...
         ?>
         <tr>
          <td class="ui-state-highlight smallText" align="right"><?php echo AnnounceVersion($contribPath, $currentVersion, $contribName); ?></td>
         </tr>
         ...
         ?>
         <tr>
          <td class="ui-state-highlight smallText" align="right"><INPUT TYPE="radio" NAME="version_check" onClick="this.form.submit();"><?php echo TEXT_VERSION_CHECK_UPDATES; ?></td>
         </tr>
         </form>
         <?php } } else { ?>
            <tr>
               <td class="ui-state-highlight smallText" align="right"><?php echo TEXT_MISSING_VERSION_CHECKER; ?></td>
            </tr>
         <?php } ?>
        </table></td>
       </tr>
       ...
       <tr>
        <td class="ui-widget-content" colspan="2"><?php echo TEXT_PAGE_TAGS; ?></td>
       </tr>
       <tr>
        <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '100%', '10'); ?></td>
       </tr>       
       ...
       <tr>
        ...
        <td align="right" width="60%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
         <?php echo tep_draw_form('header_tags', FILENAME_HEADER_TAGS_SEO, '', 'post') . tep_draw_hidden_field('action', 'update'); ?>
          <tr>
           <td class="ui-widget-content" height="60" valign="top"><?php echo TEXT_INFORMATION_PAGES; ?></td>
          </tr>
            ...
            <tr>
             <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
               <tr>
                <td><table border="0" width="100%">
                 <tr>
                  <td class="ui-widget-header ui-corner-all smallText" width="25%" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_PAGENAME; ?></td>
                  <td class="ui-widgett-content"align="left"><?php echo tep_draw_pull_down_menu('new_files', $newfiles, '', 'onChange="this.form.submit();"', false);?></td>
                 <tr>             
                </table></td>
               </tr>          
               <?php
                    ...
                    <td><table border="2" width="100%">
                     <tr>
                      <td ><table border="0" style="background-color: #f0f1f1;" width="100%" class="infoBoxContent">
                       <tr>
                        <th class="ui-widget-header smallText"width="50%"><?php echo $newfiles[$x]['text'] . ' - '. $languages[$i]['name']; ?></th>
                        <td align="right"><table class="ui-widget-content" border="0" cellpadding="0" celspacing="0">
                        <tr> 
                     ...
                     <tr>            
                      <td><table border="1" width="100%">
                       <tr>
                        <td class="ui-state-default smallText" width="18%" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_TITLE; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['title']; ?>" class="popup" >
                         <div style="float:left; width:100%;">
                          <div style="float:left;">
                           <input type="text" name="title_<?php echo $id; ?>" value="<?php echo ($pageTags['page_title']) ? $pageTags['page_title'] : ''; ?>" maxlength="255" size="60" id="pagetitle_<?php echo $id_toggle; ?>"                             
                       ...
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_DESCRIPTION; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['desc']; ?>" class="popup" >
                         <div style="float:left; width:100%;">
                          <div style="float:left;">                        
                           <input type="text" name="desc_<?php echo $id; ?>" value="<?php echo ($pageTags['page_description']) ? $pageTags['page_description'] : ''; ?>" maxlength="255" size="60" id="pagedesc_<?php echo $id_toggle; ?>"'
                             onkeyup="javascript:return ShowCharacterCount('<?php echo 'pagedesc_' . $id_toggle; ?>')">  
                          </div> 
                       ...
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_KEYWORDS; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['keywords']; ?>" class="popup" >
                         <input type="text" name="keyword_<?php echo $id; ?>" value="<?php echo ($pageTags['page_keywords']) ? $pageTags['page_keywords'] : ''; ?>" maxlength="255" size="60" id="keyword_<?php echo $id_toggle; ?>"'>
                       ...
                       <tr style="background: <?php echo (tep_not_null($pageTags['alt_1']) ? "#99ffff" : white) ?>;">
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_LOGO; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['logo']; ?>" class="popup" ><input type="text" name="logo_<?php echo $id; ?>" value="<?php echo ($pageTags['page_logo']) ? $pageTags['page_logo'] : ''; ?>" maxlength="255" size="60" id="logo_<?php echo $id_toggle; ?>"'>
                       ...
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_OPTIONS; ?></td>
                        <td class="ui-widget-content smallText"><table class="ui-state-default" border="2" width="100%">
                          <?php 
                          ...
                          ?> 
                           <tr onMouseOver="this.style.backgroundColor='#F0F0F0'" onMouseOut="this.style.backgroundColor='#FFFFFF'" bgcolor="#FFFFFF">
                            <td><table border="0" class="ui-state-default" ><!-- style="border: ridge 1 px; width: 100%"> -->
                              <tr>
                           ...
                           $sortNumb = (tep_not_null($pageTags[$sortoptTag]) && $pageTags[$sortoptTag] !== '0') ? $pageTags[$sortoptTag] : '';                              
                            ...
                            <td><table border="0" class="ui-state-default"> <!-- style="border: ridge 1 px; width: 100%">-->
                             <tr>
                              ...
                              <td align="left" width="10" align="right" title="<?php echo $optionPopup[$z + $incrCheckNumb]; ?>" class="ui-widget-content popup"><input type="checkbox" name="option_<?php echo $z . '_' . $id; ?>" id="option_<?php echo $z . '_' . $id; ?>" <?php echo $checked; ?> onClick="UpdateSortOrder('<?php echo $z . '_'. $id; ?>');"></td>
                              <td class="smallText" align="right" title="<?php echo $optionPopup[$z + $incrSortNumb]; ?>" class="ui-widget-content popup"><?php echo tep_draw_input_field('sortoption_'.$z . '_'. $id, $sortNumb, 'maxlength="2" size="1" ' . $disabled . ' id="' . $z . '_' . $id . '"'); ?> </td>
                             </tr>
                            </table></td> 
                           <?php } else { ?>
                     ...
                     <tr>            
                      <td><table border="0" width="100%" id="<?php echo $id_toggle; ?>" style="display: <?php echo (($id_toggle == $showMetaInfoItem) ? 'inline' : 'none'); ?>">
                       <tr class="smallText" style="background: <?php echo (tep_not_null($shopsMetaInfo['title']) ? yellow : red) ?>;">
                       ...
                       <tr class="smallText" style="background: <?php echo (tep_not_null($shopsMetaInfo['description'][1]) ? yellow : red) ?>;">
                       ...
                       <tr class="smallText" style="background: <?php echo (tep_not_null($shopsMetaInfo['keywords'][1]) ? yellow : red) ?>;">
        ...
        <td align="right" width="40%" valign="top"><table class="ui-widget-header ui-corner-all" width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
          <td align="right" width="40%" valign="top"><table class="ui-widget-content ui-corner-all" width="100%" border="0" cellspacing="0" cellpadding="0">
           <?php echo tep_draw_form('header_tags_default', FILENAME_HEADER_TAGS_SEO, '', 'post') . tep_draw_hidden_field('action', 'default'); ?>
            <tr>
             <td class="ui-widget-content" smallText" height="60" valign="top"><?php echo TEXT_INFORMATION_DEFAULT; ?></td>
            </tr>          
            <tr>
             <td valign="top" width="100%"><table class="ui-widget-header ui-corner-all" width="100%" border="2">             
              <tr>
               <td width="100%" valign="top"><table class="ui-widget-content ui-corner-all" border="0" width="100%" cellspacing="0" cellpadding="0">
                 <!-- BEGIN DEFAULT TAGS -->      
                 <?php             
                  $numbLanguages = count($languages);
                  for ($i=0; $i < $numbLanguages; ++$i) { //show one for each language
                    ...
                    <td><table border="1" width="100%" >
                     <tr>
                      <td><table border="0" width="100%" class="infoBoxContent">
                       <tr>
                        <th class="ui-widget-header ui-corner-all smallText" style="background-color: #f0f1f1; text-align: center;"><?php echo HEADING_TITLE_SEO_DEFAULT_TAGS . ' - '. $languages[$i]['name']; ?></th>
                       </tr> 
                      </table></td>
                     </tr>
                     <tr>            
                      <td><table border="1" width="100%">
                       <tr>
                        <td class="ui-state-default smallText" width="25%" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_TITLE; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['def_title']; ?>" class="popup" ><input type="text" name="default_title_<?php echo $id; ?>" value="<?php echo ($defaultTags['default_title'] ?: ''); ?>" maxlength="255" size="36"'> </td>
                       </tr> 
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_DESCRIPTION; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['def_desc']; ?>" class="popup" ><input type="text" name="default_desc_<?php echo $id; ?>" value="<?php echo ($defaultTags['default_description'] ?: ''); ?>" maxlength="255" size="36"'> </td>
                       </tr> 
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_KEYWORDS; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['def_keywords']; ?>" class="popup" ><input type="text" name="default_keyword_<?php echo $id; ?>" value="<?php echo ($defaultTags['default_keywords'] ?: ''); ?>" maxlength="255" size="36"'> </td>
                       </tr>
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_LOGO; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['def_logo']; ?>" class="popup" ><input type="text" name="default_logo_text_<?php echo $id; ?>" value="<?php echo ($defaultTags['default_logo_text'] ?: ''); ?>" maxlength="255" size="36"'> </td>
                       </tr>
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold; vertical-align:top"><?php echo HEADING_TITLE_SEO_HOME_PAGE; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['def_home_text']; ?>" class="popup" >
                        
                         <?php
                         if (HEADER_TAGS_ENABLE_HTML_EDITOR == 'CKEditor') { 
                             echo tep_draw_textarea_field('home_page_text_' . $id, 'soft', 37, 8, ($defaultTags['home_page_text'] ?: ''), 'id = "home_page_text_' . $id . '" class="ckeditor"');
                         } else { 
                             echo tep_draw_textarea_field('home_page_text_' . $id, 'soft', 36, 8, ($defaultTags['home_page_text'] ?: ''));
                         }                       
                         ?>
                        </td>
                       </tr>                                               
                       <tr>
                        <td class="ui-state-default smallText" style="font-weight: bold;"><?php echo HEADING_TITLE_SEO_OPTIONS; ?></td>
                        <td class="ui-widget-content smallText"><table border="0" width="100%">
                         <tr>
                          <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
                           <tr>
                            <td><table border="0" class="ui-state-default">
                             <tr>
                              <?php
                              for ($y = 0; $y < count($def_options); ++$y) {
                              ...
                              <td class="smallText" width="90" title="<?php echo $defaultPopup[$y]; ?>" class="ui-widget-content popup"><?php echo $def_options[$y]; ?></td>
                              <?php
                               if ( $y < 2)
                               {
                                 if ( $y == 1)
                 ...
                 <tr>
                  <td><table border="2" width="100%">
                   <tr>
                    <td><table border="0" width="100%" class="infoBoxContent">
                     <tr>
                      <th class="ui-widget-header smallText" ><?php echo HEADING_TITLE_SEO_DEFAULT_META_TAGS; ?></th>
                     </tr> 
                    </table></td>
                   </tr>              
                   <tr>            
                    <td><table class="ui-widget-content ui-corner-all" border="0" width="100%">               
                     <tr>
                      <td><table class="ui-widget-content" border="0" width="100%">
                       <tr>
                       <?php
                        ...
                        ?>
                        <td class="ui-state-default" align="left" title="<?php echo $metatagsPopup[$y]; ?>" class="popup"><?php echo tep_draw_checkbox_field('metatags_'.$y, '', $checked, ''); ?> </td>
                        <td align="left" class="smallText" width="90" title="<?php echo $metatagsPopup[$y]; ?>" class="popup"><?php echo $metaTags[$y]; ?></td>
                        <?php 
                         if ( $y < 3)
                         {
                           if ( $y == 2)
         ...
         <tr>
          <td align="right" width="40%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
           <?php echo tep_draw_form('header_tags_pseudo', FILENAME_HEADER_TAGS_SEO, '', 'post') . tep_draw_hidden_field('action', 'default'); ?>
            <tr>
             <td valign="top" width="100%"><table width="100%" border="2">             
              <tr>
               <td width="100%" valign="top"><table class="ui-widget-header ui-corner-all" border="0" width="100%" cellspacing="0" cellpadding="0">
                 <!-- BEGIN PSEUDO CODE -->      
                 <tr>
                  <td><table class="ui-widget-content" border="2" width="100%">
                   <tr>
                    <td><table border="0" width="100%" class="infoBoxContent">
                     <tr>
                      <th class="ui-widget-header smallText" style="background-color: #f0f1f1; text-align: center;"><?php echo HEADING_TITLE_SEO_PSEUDO; ?></th>
                     </tr> 
                    </table></td>
                   </tr>              
                   <tr>            
                    <td><table border="0" width="100%" cellpadding="0">               
                     <tr>
                      <td class="smallText"><table border="1" width="100%">
                       <tr>
                        <td class="ui-widget-content smallText" colspan="2"><?php echo TEXT_PSEUDO_PAGE_NAME_NOTE; ?></td>
                       </tr>                
                       <tr>
                        <td class="ui-state-default smallText" width="27%" style="font-weight: bold;"><?php echo TEXT_PSEUDO_PAGE_NAME; ?></td>
                        <td class="ui-widget-content smallText" title="<?php echo $commonPopup['pseudo_add']; ?>" class="popup" ><input type="text" name="pseudo_page_name" maxlength="255" size="34"></td>
                       </tr>
                      </table></td>
                     <tr>
                    </table></td>
