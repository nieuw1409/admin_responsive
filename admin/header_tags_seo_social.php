    ...
    <td><table border="0" cellpadding="0" width="100%">
      ...
      <?php     
        ...
        foreach ($iconsArray as $group => $key) {
            echo '<div class="main" id="group_names" style="float:left; font-weight:bold; padding-right:10px;"><INPUT TYPE="radio" NAME="size_group" VALUE="' . $group . '"' . ' id="' . $group . '"' . ($group == $checked[$group] ? ' checked ' : '') . ' onclick="javascript:return ChangeIconSize()" />' . $group .'</div>';
        }    
        echo '<div style="float:left;"><input type="checkbox" id="disable_social"' . ($db['configuration_value'] == 'false' ? 'checked' : '') . ' />' . TEXT_DISABLE . '</div>';
        echo '</div>'; 
