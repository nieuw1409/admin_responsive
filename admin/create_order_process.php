...
*/
 ...
 // if ($_POST['action'] != 'process') {
  ...
  //}
  $customers_id = (empty($_POST['customers_id']) ? 10 : (int)tep_db_prepare_input($_POST['customers_id']));
...
// eof multis tores
  ...
  if ($_POST['customers_create_type'] == 'new') {
	...
	$sql_data_array = array('customers_firstname' => $firstname,
							...
							'customers_stores_id' => $stores_id); // multi stores
    ...
    $sql_data_array = array('customers_id' => $customers_id,
							...
							'billing_stores_id' => $stores_id         // multi stores
							); 
  ...
  $insert_id = tep_db_insert_id();
    ...
    tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
      ...
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && tep_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
        ...
        $co_modules = array();
	    $include = array( 'ot_total', 'ot_subtotal', 'ot_subtotal_ex', 'ot_tax', 'ot_loworderfee' ) ;
        ...
        foreach($order_total_modules as $key => $value) {
          ...
          if ($co_modules[$class]->enabled) {
		     ...
		     if ( SHIPPING_CREATE_ORDER == 'all' ) {
			   $add_this = true ;
		     } 
		     if ( in_array( $co_modules[$class]->code, $include ) ) {
			   $add_this = true ;
		     }
		     if ( SHIPPING_CREATE_ORDER == $co_modules[$class]->code ) {
			   $add_this = true ;
		     }
