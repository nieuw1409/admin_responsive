   ...
   if (isset($_POST[0])) {
       ...
       switch ($_POST[0]) {
           ...
           case $_POST[0] == 'sizechange':
               ...
               switch ($name) {
                   case TEXT_SIZE_16: $maxCol = 55; break;
                   case TEXT_SIZE_24: $maxCol = 35; break;
                   case TEXT_SIZE_32: $maxCol = 25; break;
                   case TEXT_SIZE_48: $maxCol = 15; break;
               }
              ...
              case $_POST[0] == 'pagination':
                  ...
                  $page_number = filter_var($page, FILTER_SANITIZE_NUMBER_INT, FILTER_FLAG_STRIP_HIGH) - 1; //sanitize post value

                  if(!is_numeric($page_number)){die('Invalid page number!');} //validate page number is really numaric

                  $position = ($page_number * MAX_DISPLAY_SEARCH_RESULTS); //get current starting point of records
                  $orderBy =  (tep_not_null($_POST[3]) ? $_POST[3] . ', language_id ASC ' : ' language_id ASC ');
                  $where = $_POST[4];
                  ...
                  if (tep_db_num_rows($keywords_query) > 0 ) {
                      ...
                      while ($keywords = tep_db_fetch_array($keywords_query)) {
                          ...
                          $keyword_color = $keywords['found'] ? '#0000FF' : ($assignedID ? '#A54BFF' : '#808000');
