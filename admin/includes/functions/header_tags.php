...
function AddMissingPages($languages_id = 1, $languages) {
	GLOBAL $multi_stores_id ;
    ($dir = opendir(DIR_FS_CATALOG)) || die(sprintf(ERROR_FAILED_DIR_OPEN, DIR_FS_CATALOG));
    $newfiles = array();
    $fileList = array();
    $existingInDB = array();

    $pageTags_query = tep_db_query("select page_name from " . TABLE_HEADERTAGS . ' where language_id = ' . (int)$languages_id . ' and stores_id = ' . (int)$multi_stores_id );
...
// Begin iOSC Mobile
  ...
  $excludeFilesList[] = 'mobile_popup_image.php';  // my shop
  $excludeFilesList[] = 'mobile_popup_images.php';  
  $excludeFilesList[] = 'mobile_popup_search_help.php';
  $excludeFilesList[] = 'mobile_popup_shipping.php';    // my shop
  $excludeFilesList[] = 'mobile_ssl_check.php';  
  $excludeFilesList[] = 'mobile_about.php' ;
  $excludeFilesList[] =  'mobile_advanced_search_result.php' ;
  $excludeFilesList[] = 'mobile_catalogue.php' ;
  $excludeFilesList[] =  'mobile_conditions.php' ;
  $excludeFilesList[] =  'mobile_contact_us.php' ;
  $excludeFilesList[] =  'mobile_index.php' ;
  $excludeFilesList[] =  'mobile_currencies.php' ;
  $excludeFilesList[] = 'mobile_languages.php' ;
  $excludeFilesList[] =  'mobile_products.php' ;
  $excludeFilesList[] =  'mobile_privacy.php' ;
  $excludeFilesList[] =  'mobile_product_info.php' ;
  $excludeFilesList[] =  'mobile_product_reviews.php' ;
  $excludeFilesList[] =  'mobile_product_reviews_write.php' ;
  $excludeFilesList[] =  'mobile_product_thumb.php' ;
  $excludeFilesList[] =  'mobile_search.php' ;
  $excludeFilesList[] =  'mobile_shipping.php' ; 

  $excludeFilesList[] =  'mobile_advanced_search_result.php' ; 
  $excludeFilesList[] =  'mobile_catalogue.php' ; 
  $excludeFilesList[] =  'mobile_conditions.php' ; 
  $excludeFilesList[] =  'mobile_contact_us.php' ; 
  $excludeFilesList[] =  'mobile_shopping_cart.php' ;   
  $excludeFilesList[] =  'mobile_shipping.php' ;   
// End iOSC Mobile  
  ...
  $isTemplate = IsTemplate(); 
    ...
    $pseudoFiles = array();
    $db_query = tep_db_query("select page_name from " . TABLE_HEADERTAGS . " where page_name LIKE '%?%' and stores_id = '" . (int)$multi_stores_id . "'");
   ...
   if ($notDuplicatePage) {
     ...
     if ($fileMatch && strpos($file, " ") !== FALSE) {  //filename contains an illegal space
      ...
      if (! $fileMatch) {                     //couldn't find a match
         global $messageStack;
         $fix = '<a href="' . tep_href_link('headertags_seo_exclude.php', 'exclude=' . $file) . '"><span style="color:#00dd00; font-size:10ps;">exclude</span></a>';
...
{   
   ...
   { 
     ...
     unset($fp[$beginSection + 2]);
        ...
        $incArray['pseudo'] .= "    if (! ReadCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['SCRIPT_FILENAME']), \$getID)) {" . "\n";
        $incArray['pseudo'] .= "      if (isset(\$parts[0])) {" . "\n";
        $incArray['pseudo'] .= "       \$found = false;" . "\n";
        $incArray['pseudo'] .= "       \$name = FILENAME_" . $fileUpper . " . \"?\" . \$parts[0] . \"=\";" . "\n";
        $incArray['pseudo'] .= "       \$pageTags_query = tep_db_query(\"select * from \" . TABLE_HEADERTAGS . \" where page_name like '\" . tep_db_input(\$name) . \"%' and language_id = '\" . (int)\$languages_id . \"'\ and stores_id = '\" . (int)\$stores_id . \"'\");" . "\n";
        ...
        $incArray['pseudo'] .= "               WriteCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['SCRIPT_FILENAME']), \$getID);" . "\n";
        ...
        $incArray['line'] = sprintf("        \$header_tags_array = tep_header_tag_page(FILENAME_%s);\n",$fileUpper );
        $incArray['line'] .= "        WriteCacheHeaderTags(\$header_tags_array, basename(\$_SERVER['SCRIPT_FILENAME']), \$getID);" . "\n";
        $incArray['line'] .= "      }  "  . "\n";    
    }
...
/********************************************************************************
Language ID's may not be contigous and the array is not sorted so this
...
function GetFileList($languages_id = 1)
{
 ...
 if ($handle = @opendir(DIR_FS_CATALOG)) 
 {
    ...
    { 
       ...
       {
          $pageTags_query = tep_db_query("select * from " . TABLE_HEADERTAGS . " where page_name like '" . tep_db_input($file) . "' and language_id = '" . (int)$languages_id . "' and stores_id = '" . (int)$multi_stores_id . "'");
    ...
    $pageTags_query = tep_db_query("select * from " . TABLE_HEADERTAGS . " where page_name like '%?%' and language_id = '" . (int)$languages_id . "' and stores_id = '" . (int)$multi_stores_id . "'");
    while ($pageTags = tep_db_fetch_array($pageTags_query))
