...
if (tep_db_num_rows($db_query) && ! isset($_GET['reset'])) {
    ...
    $db_sql_array[] = array("INSERT INTO database_optimizer (last_update, customers_last_update, orders_last_update, sessions_last_update, user_tracking_last_update) VALUES ('2011-01-01', '2011-01-01', '2011-01-01', '2011-01-01', '2011-01-01')");
    ...
    VALUES
      ...
      ('Truncate Customers Old',         'DATABASE_OPTIMIZER_CUSTOMERS_OLD',          '60',   'Should older entries in the customers and address book tables be removed? Enter the number of days between removals or leave blank for no removal. (<b>Value entered must be in days</b>)', '" . $configuration_group_id . "', '30', NULL, NOW(), NULL),
      ('Truncate Product Notifications', 'DATABASE_OPTIMIZER_PRODUCT_NOTIFICATIONS',  '60',   'Should entries in the product notification table be deleted if the customer and/or product no longer exist?  (<b>Value entered must be in days</b>)', '" . $configuration_group_id . "', '33', NULL, NOW(), NULL),
      ...
      ('Truncate User Tracking',         'DATABASE_OPTIMIZER_USER_TRACKING',          '3',    'Should older entries in the user tracking table be removed? Enter the number of days between removals or leave blank for no removal. (<b>Value entered must be in days</b>)', '" . $configuration_group_id . "', '55', NULL, NOW(), NULL),
      ('Email Notification',             'DATABASE_OPTIMIZER_EMAIL_NOTIFY',           'test@myaddress.com', 'Enter the email address that you would like the email sent to. Enter multiple email addresses by separating them with a comma. A blank entry means no emails will be sent.', '" . $configuration_group_id . "', '60', NULL, NOW(), NULL),
