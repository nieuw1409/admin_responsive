...
// class constructor
    function paymentModuleInfo($pmInfo_array) {
      ...
      for ($i = 0, $n = sizeof($pmInfo_array) - 1; $i < $n; $i++) {
        $key_value_query = tep_db_query("select configuration_title, configuration_value, configuration_description from " . $multi_stores_config . " where configuration_key = '" . $pmInfo_array[$i] . "'");
