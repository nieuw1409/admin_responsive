...
// 232    $check_customer_query = tep_db_query("select customers_firstname, customers_lastname, customers_password, customers_id from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($email_address) . "'");
...
// 232      $new_password = tep_create_random_value(ENTRY_PASSWORD_MIN_LENGTH);
// 232      $crypted_password = tep_encrypt_password($new_password);
...
// 232		tep_db_query("update " . TABLE_CUSTOMERS . " set customers_password = '" . tep_db_input($crypted_password) . "' where customers_id = '" . (int)$check_customer['customers_id'] . "'");
      ...
      if (EMAIL_USE_HTML == 'true'){
        $text_query = tep_db_query("SELECT * FROM " . TABLE_EMAIL_ORDER_TEXT . " where eorder_text_id = '" . _SYS_EMAIL_HTML_PASSWORDFORGOTTEN . "' and language_id = '" . $languages_id . "' and stores_id='" . SYS_STORES_ID . "'");	
      } else{
        $text_query = tep_db_query("SELECT * FROM " . TABLE_EMAIL_ORDER_TEXT . " where eorder_text_id = '" . _SYS_EMAIL_NORMAL_PASSWORDFORGOTTEN . "' and language_id = '" . $languages_id . "' and stores_id='" . SYS_STORES_ID . "'");
      }
      ...
      $text = preg_replace('/<-SYS_CUSTOMER_FIRSTNAME->/',       $check_customer['customers_firstname'], $text);
      $text = preg_replace('/<-SYS_CUSTOMER_LASTNAME->/',        $check_customer['customers_lastname'], $text);	  
      $text = preg_replace('/<-SYS_CUSTOMER_EMAILADDRESS->/',    $email_address, $text);
// 232      $text = preg_replace('/<-SYS_NEW_PASSWORD->/',             $new_password , $text);	
   
 	  $text = preg_replace('/<-SYS_RESET_PASSWORD_URL->/',      '<a href="' . $reset_key_url .'" target="_blank">' .  TEXT_PASSWORD_RESET . '</a>&nbsp;&nbsp;', $text);	  
...
//      tep_mail($check_customer['customers_firstname'] . ' ' . $check_customer['customers_lastname'], $email_address, EMAIL_PASSWORD_REMINDER_SUBJECT, sprintf(EMAIL_PASSWORD_REMINDER_BODY, $new_password), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
      tep_mail($check_customer['customers_firstname'] . ' ' . $check_customer['customers_lastname'], $email_address, $subject, $email_text, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);

      $messageStack->add_session('login', SUCCESS_PASSWORD_SENT, 'success');

// 232      tep_redirect(tep_href_link(FILENAME_LOGIN, '', 'SSL'));
...
<div class="contentContainer">
  ...
  <div class="buttonSet">
    <span class="buttonAction"><?php echo tep_draw_button(SMALL_IMAGE_BUTTON_SEND_EMAIL, 'chevron-right', null, 'primary'); ?></span>

    <?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'chevron-left', tep_href_link(FILENAME_LOGIN, '', 'SSL')); ?>
  </div>
</div>
...
} // } ELSE { FROM if ($password_reset_initiated == true) {
