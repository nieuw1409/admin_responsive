...
//    $customer_info_query = tep_db_query("select c.customers_firstname,                                                                                                           c.customers_default_address_id, ab.entry_country_id, ab.entry_zone_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " ab on (c.customers_id = ab.customers_id and c.customers_default_address_id = ab.address_book_id) where c.customers_id = '" . (int)$login_customer_id . "'");
    $customer_info_query =   tep_db_query("select c.customers_firstname, c.customers_group_id, c.customers_password, c.customers_email_address, c.customers_specific_taxes_exempt, c.customers_default_address_id, ab.entry_country_id, ab.entry_zone_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " ab on (c.customers_id = ab.customers_id and c.customers_default_address_id = ab.address_book_id) where c.customers_id = '" . (int)$login_customer_id . "'");
...
// BOF Separate Pricing Per Customer
	...
	$check_customer_group_tax = tep_db_query("select customers_group_show_tax, customers_group_tax_exempt, group_specific_taxes_exempt from " . TABLE_CUSTOMERS_GROUPS . " where customers_group_id = '" .(int)$customer_info['customers_group_id'] . "'");
...
// bof contact us form
    $customer_email_ref = $customer_info['customers_email_address'];
    tep_session_register('customer_email_ref');
// eof contact us form	
