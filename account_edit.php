...
// needs to be included earlier to set the success message in the messageStack
  ...
  if (isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'process') && isset($HTTP_POST_VARS['formid']) && ($HTTP_POST_VARS['formid'] == $sessiontoken)) {
    ...
    if (ACCOUNT_DOB == 'true') {
//      if ((is_numeric(tep_date_raw($dob)) == false) || (@checkdate(substr(tep_date_raw($dob), 4, 2), substr(tep_date_raw($dob), 6, 2), substr(tep_date_raw($dob), 0, 4)) == false)) { // 2.3.3.3
...
?>
  ...
  <div class="col-xs-9">  
    <div class="form-group has-feedback">
        <?php echo tep_bs_radio_field('gender', 'm', MALE,     'input_Cust_gender_male',   $male,   'radio radio-success radio-inline', '', '', 'right', 'required aria-required="true"') ; ?>
        <?php echo tep_bs_radio_field('gender', 'f', FEMALE,   'input_Cust_gender_female', $female, 'radio radio-success radio-inline', '', '', 'right', 'required aria-required="true"') ; ?>
    </div>		
  </div>
  <?php
  }
...
?>
  ...
  <div class="buttonSet">
    ...
    <?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'chevron-left', tep_href_link(FILENAME_ACCOUNT, '', 'SSL')); ?>
  </div>
</div>
