...
define('FEEDNAME', 'product_feed.txt');       //from your googlebase account
define('DOMAIN_NAME', $_SERVER['SERVER_NAME'] ); //your correct domain name (don't include www unless it is used but do include the shops directory)
...
define('CURRENCY_TYPE', LANGUAGE_CURRENCY);  //(eg. USD, EUR, GBP)
...
define('OPTIONS_FEED_LANGUAGE', 'NLD');
...
define('OPTIONS_GOOGLE_PRODUCT_CATEGORY_STANDARD', '' ) ;// same as above only is no code is filled in products table while db is active this code will be used
define('OPTIONS_GTIN', '');
define('OPTIONS_ISBN', '');
...
define('OPTIONS_SHIPPING_STRING', 'NL:NL:Ground:7.00'); //says charge shipping to US for residents of Florida at 5% and don't apply tax to shipping
...
AND find_in_set('" . SYS_STORES_ID . "', products.products_to_stores) != 0 
AND find_in_set('" . _SYS_STORE_CUSTOMER_GROUP . "', products.products_hide_from_groups) = 0 
...
AND categories_description.language_id = " . DEFAULT_LANGUAGE_ID . "
AND find_in_set('" . SYS_STORES_ID . "', categories.categories_to_stores) != 0 
AND find_in_set('" . _SYS_STORE_CUSTOMER_GROUP . "', categories.categories_hide_from_groups) = 0  
AND categories_status = '1' ";
...
if(OPTIONS_ENABLED == 1) {
   ...
   if(OPTIONS_ENABLED_GOOGLE_PRODUCT_CATEGORY == 1) $output .= "\tgoogle_product_category";
...
//            $output .= "\t" . (OPTIONS_GOOGLE_PRODUCT_CATEGORY == 'db' ? $row->google_category. 'bestand' : OPTIONS_GOOGLE_PRODUCT_CATEGORY . 'std');
            if ( OPTIONS_GOOGLE_PRODUCT_CATEGORY == 'db' ) {
              $products_google_query = tep_db_query("select google_taxonomy_number from " . TABLE_PRODUCTS_GOOGLE_TAXONOMY . " where google_taxonomy_id = '" . $row->google_category . "'");
              $products_google_array = tep_db_fetch_array($products_google_query); 
              if ( tep_not_null($products_google_array['google_taxonomy_number'] ) ) { 
                 $output .= "\t" . $products_google_array['google_taxonomy_number'] ;
			  } else {
                $output .= "\t" . OPTIONS_GOOGLE_PRODUCT_CATEGORY_STANDARD ;			     
			  }
			} else {
              $output .= "\t" . OPTIONS_GOOGLE_PRODUCT_CATEGORY ;
			}
		 }
...
//(isset($_GET['noftp']) ? TEXT_FEEDERS_GOOGLE : TEXT_FEEDERS_GOOGLE_NOFTP) . '</div>' . PHP_EOL ;
...
echo '<p style="margin:auto; text-align:left">';
printf( 'group ' . _SYS_STORE_CUSTOMER_GROUP . ' store ' . SYS_STORES_ID . " Feed contains %d products.", $statsArray['total'] );
echo '</p>';
...
$completed = 'store id '. SYS_STORES_ID . ' File Completed' . ($warning ? ' (with warnings): ' : ':' );
